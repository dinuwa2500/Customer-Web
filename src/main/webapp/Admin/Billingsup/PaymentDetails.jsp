...
<%@page import="dbc.PaymentDbcImpl"%>
<%@page import="db.DBconnect"%>
<%@page import="java.util.List"%>
<%@page import="dbc.PaymentDbc"%>
		...
		<main>
			<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
				...
				if (userId == null || uemail == null) {
                    out.println("<p>Error: User not logged in or role not assigned.</p>");
                    return;
                }

                // Allow access only to "admin", "technical_support", and "billing_support"
                if (!uemail.equals("admin@gmail.com") && !uemail.equals("Techsup@gmail.com") && !uemail.equals("Billingsup@gmail.com")) {
                    out.println("<p>Access denied. You do not have permission to view this page.</p>");
                    return;
                }
				
					
				
				PaymentDbcImpl payimp = new PaymentDbcImpl();
				
				DBconnect.getcon();
				
	
				// Retrieve the list of packages
				List<Payment> Payments = payimp.getAllPay();
				%>
					...
					<table class="table table-striped table-hover">
						<thead class="table-dark">
							<tr>
								...
								<th scope="col">Expire Month & Year</th>
						...
						<tbody>
							...
							%>
							<tr>
								<th scope="row"><%=pay.getPayid()%></th>
								<td><%=pay.getCardname()%></td>
								<td><%=pay.getCardnumber()%></td>
								<td><%=pay.getExpirydate()%></td>
								<td><%=pay.getAmount()%></td>
						
								<td>
                     
                       <a href = "${pageContext.request.contextPath}/deletePayment?id=<%=pay.getPayid()%>"><button class="btn btn-danger btn-sm">Delete</button></a>
                    </td>
                </tr>
            <%
            } // End of for loop
